...
async fn run() -> Result<()> {
    ...
    loop {
        ...
        for (i, entry) in wallets.iter().enumerate() {
            ...
            if cfg.chainstreak_enabled() {
                if did_any { println!(); }
                match categories::chainstreak::run(&client, nonce).await {
                    Ok(n) => nonce = n,
                    Err(_) => lr("Chainstreak category failed and continued"),
                }
                did_any = true;
            }

            if cfg.chainsgreets_enabled() {
                if did_any { println!(); }
                match categories::chainsgreets::run(&client, nonce).await {
                    Ok(n) => nonce = n,
                    Err(_) => lr("Chainsgreets category failed and continued"),
                }
                did_any = true;
            }

            if cfg.paymearc_enabled() {
                if did_any { println!(); }
                match categories::paymearc::run(&client, nonce).await {
                    Ok(n) => nonce = n,
                    Err(_) => lr("Payme Arc category failed and continued"),
                }
                did_any = true;
            }

            if cfg.inuxarc_enabled() {
                if did_any { println!(); }
                match categories::inuxarc::run(&client, nonce).await {
                    Ok(n) => nonce = n,
                    Err(_) => lr("Inuxarc category failed and continued"),
                }
                did_any = true;
            }

            if cfg.yarcgmgn_enabled() {
                if did_any { println!(); }
                match categories::yarcgmgn::run(&client, nonce).await {
                    Ok(n) => nonce = n,
                    Err(_) => lr("Yarcgmgn category failed and continued"),
                }
                did_any = true;
            }
